...
target("lcd_st7735_lib_am")
    ...
    add_includedirs(
      path.join(lcd_dir,"st7735"),
      path.join(lcd_dir,"core"),
      {public = true}
    )

target("legacy_drivers")
    set_kind("static")
    add_files("ksz8851.c", "lcd.c", "adc.c", "ksz8851.c")
    add_includedirs(
      "./",
      path.join(legacy_dir, "common"),
      {public = true}
    )
